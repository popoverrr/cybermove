...
for (const size of opt.sizes) {
  ...
  if (mobile) {
    // флик 2200 px за 250 мс: 10 шагов по 220 px каждые 25 мс
    for (let i = 0; i < 10; i++) {
  ...
  summary.flick = { px: mobile ? 2200 : 1440, transitions: transitions(flickFrames), flips: analyze(flickFrames, mobile ? 4.5 : 7).screenFlips };
