/**
 * S7 · РОСТ — GROWTH (BRIEF §7 S7). Тема ivory.
 * Атом возвращается крупнее, орбит 13, они расходятся как кольца роста (волосяные линии taupe).
 * Выход: орбиты схлопываются в спокойный атом, фон переходит в sand (S8).
 */
...
export class GrowthScene implements SceneModule {
  readonly id = 'growth';
  readonly range = { enter: 0.25, exit: 0.82 };
  init() {}
