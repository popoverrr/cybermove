...
for (const s of SIZES) {
  ...
  const headerInk = await page.evaluate(async (headerH) => {
  ...
  const line = `${s.name}: наложение карточек на счётчики ${r.overlapPx.toFixed(0)} px, обрезано снизу ${r.clippedPx.toFixed(0)} px, первая карточка x ${r.firstCardLeft === null ? '—' : r.firstCardLeft.toFixed(0)}, тёмных пикселей в полосе шапки ${headerInk ? headerInk.dark : '—'}`;
