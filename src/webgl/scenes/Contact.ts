/**
 * S8 · КОНТАКТ (BRIEF §7 S8). Тема sand. Слева форма, справа спокойный атом. Низкая активность.
 * Фокус в поле ускоряет электроны; успешная отправка даёт мягкую волну светлого от ядра (без свечения).
 */
...
export class ContactScene implements SceneModule {
  readonly id = 'contact';
  readonly range = { enter: 0.6, exit: 1 };
  private focus = 0;
  private pulse = 0;
  init() {
  ...
  update(rig: Rig, local: number, dt: number, time: number, e: Engine) {
    ...
    rig.coreEmissive = this.pulse * 0.9;
    ...
    rig.particles.size = 1.2 + this.pulse * 0.4;
